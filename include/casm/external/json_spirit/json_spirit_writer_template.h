...
namespace json_spirit
{
    ...
    {
        os << std::showpoint << std::setprecision( precision ) << std::fixed << d;
    }
    ...
    public:
        ...
        Generator( const Value_type& value, Ostream_type& os, unsigned int indent, unsigned int prec, unsigned int options )
        :   os_( os )
        ,   indentation_size_( indent )
    ...
    private:
        ...
        void output( const Value_type& value )
        {
            switch( value.type() )
            {
                case obj_type:   output( value, value.get_obj() );   break;
                case array_type: output( value, value.get_array() ); break;
                case str_type:   output( value.get_str() );   break;
                case bool_type:  output( value.get_bool() );  break;
                case real_type:  output( value, value.get_real() );  break;
        ...
        void output( const Value_type &value, const Object_type& obj )
        {
        ...
        void output( const Value_type &value, double d )
        {
            ...
            else if( remove_trailing_zeros_ || value.get_remove_trailing_zeros() )
            {
                ...
                append_double( os, d, precision_ );  // note precision is 16 so that we get some trailing space that we can remove,
            ...
            {
                append_double( os_, d, precision_ );
            }
        }
        ...
        void output( const Value_type& value, const Array_type& arr )
        {
            //if( (value.get_force_row() || (single_line_arrays_ && !value.get_force_column())) && !contains_composite_elements( arr )  )
        ...
        void indent()
        {
            ...
            {
              for( unsigned int j=0; j<indentation_size_; ++j)
                os_ << " ";
            }
        }
        ...
        Ostream_type& os_;
        unsigned int indentation_size_;
    ...
    //
    template< class Value_type, class Ostream_type >
    void write_stream( const Value_type& value, Ostream_type& os, unsigned int indent, unsigned int prec, unsigned int options = 0 )
    {
        os << std::dec;
        Generator< Value_type, Ostream_type >( value, os, indent, prec, options );
    }
    ...
    typename Value_type::String_type write_string( const Value_type& value, unsigned int indent, unsigned int prec, unsigned int options = 0 )
    {
        typedef typename Value_type::String_type::value_type Char_type;

        std::basic_ostringstream< Char_type > os;

        write_stream( value, os, indent, prec, options );
